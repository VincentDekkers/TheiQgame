import cv2
import numpy as np
import files.twodimentionalomas as twodimentional
import time as t

def update(x,y,selected, pieces, grid):
    if selected[1]:
        selected[1] = 0
        arr[732:759,930:1180] = (255,255,255)
    if (200< y < 500):
...
def solve(arr, grid, pieces, placedpieces):
    ...
    if len(placedpieces) < 12:
        cv2.putText(arr, 'No solutions from this position',(930,750),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,200),2)
        selected[1] = 1
    copygrids(grid, solution)
...
def loadbesttimes():
    try:
        data = []
        with open('files/omasrecords.py','r') as file:
...
def writebesttimes(records):
    with open('files/omasrecords.py','w') as file:
